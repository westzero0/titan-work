...
const APP_VERSION = "3.3"; // 👈 기능 수정할 때마다 이 숫자를 1.6, 1.7로 올리세요!
...
// ==========================================
// 📋 [신규] 기존 일정에서 자재 추출 → 자재관리 시트 일괄 등록
// ==========================================

// 자재명 파싱: "실리콘 10개", "CD관(16호) 5M", "폼건 1대" 등에서 품명/규격/단위 추출
function parseMaterialText(text) {
    ...
    for (const part of parts) {
        // 패턴들:
        // 1. "품명(규격) 수량단위"  예: "CD관(16호) 5M", "실리콘(백) 10개"
        // 2. "품명 수량단위"        예: "실리콘 10개", "폼건 1대"
        // 3. "품명(규격)"           예: "CD관(16호)"
        // 4. "품명"                 예: "실리콘"

        let name = '', spec = '', unit = '개', qty = 1;
        ...
        // 수량+단위 패턴 찾기 (숫자+한글/영문)
        const qtyUnitMatch = part.match(/(\d+(?:\.\d+)?)\s*([가-힣a-zA-Z]+)/);
        if (qtyUnitMatch) {
            qty = parseFloat(qtyUnitMatch[1]);
            unit = qtyUnitMatch[2];
        }

        // 품명 추출: (규격) 앞부분, 또는 수량 앞부분
        let namePart = part;
        if (specMatch) {
            namePart = part.substring(0, specMatch.index).trim();
        }
        if (qtyUnitMatch) {
            namePart = namePart.substring(0, qtyUnitMatch.index).trim();
        }
        name = namePart.replace(/\([^)]*\)/, '').trim(); // 혹시 남은 괄호 제거

        if (name) {
            results.push({ name, spec: spec || '-', unit, qty });
        }
    }
...
// allSchedules에서 사용된 모든 자재 수집 (중복 제거)
function collectUsedMaterialsFromSchedules() {
    const materialMap = new Map(); // key: "name|spec|unit"

    for (const s of allSchedules) {
        if (!s.materials || s.materials === "없음") continue;

        const parsed = parseMaterialText(s.materials);
        for (const m of parsed) {
            const key = `${m.name}|${m.spec}|${m.unit}`;
            if (!materialMap.has(key)) {
                materialMap.set(key, { ...m, count: 1 });
            } else {
...
// 자재관리 시트에 일괄 등록 (중복 체크 후 신규만 추가)
async function registerUsedMaterialsToMaster() {
    // 1. 현재 마스터 데이터 로드 (이미 allMaterials에 있음)
    await loadMaterialData();

    // 2. 기존 마스터에 있는 자재들 키셋 만들기
    const existingKeys = new Set();
    for (const cat in allMaterials) {
        for (const m of allMaterials[cat]) {
            existingKeys.add(`${m.name}|${m.spec}|${m.unit}`);
        }
    }

    // 3. 일정에서 사용된 자재 수집
    const usedMaterials = collectUsedMaterialsFromSchedules();

    // 4. 신규 자재만 필터링
    const newMaterials = usedMaterials.filter(m => !existingKeys.has(`${m.name}|${m.spec}|${m.unit}`));

    if (newMaterials.length === 0) {
        alert('✅ 이미 모든 자재가 마스터에 등록되어 있습니다.');
        return;
    }
        ...
        <div style="max-height:300px; overflow-y:auto; margin-bottom:15px;">
            ${newMaterials.map(m => `
                <div style="padding:10px; border-bottom:1px solid #f1f5f9; font-size:0.9rem;">
                    <b>${m.name}</b> ${m.spec !== '-' ? `<span style="color:#64748b;">(${m.spec})</span>` : ''}
                    <span style="color:#94a3b8;">${m.qty}${m.unit}</span>
                    <span style="color:#2563eb; font-size:0.75rem; margin-left:8px;">사용 ${m.count}회</span>
                </div>
            `).join('')}
        </div>
        <p style="font-size:0.85rem; color:#64748b;">총 ${newMaterials.length}개 신규 자재를 자재관리 시트에 등록합니다.</p>
    `;

    if (!confirm(confirmHtml.replace(/<[^>]*>/g, ''))) return;
    ...
    for (const m of newMaterials) {
        try {
            ...
            if (text.includes('SUCCESS')) {
                successCount++;
                // 로컬 캐시에도 추가
                const customUid = "AUTO_" + Date.now() + "_" + Math.random().toString(36).substr(2, 5);
                const newItem = { uid: customUid, category: '기타', subCat: "기타", name: m.name, spec: m.spec, unit: m.unit, price: 0 };
                if (!allMaterials['기타']) allMaterials['기타'] = [];
                allMaterials['기타'].unshift(newItem);
            } else {
                failCount++;
                console.error('등록 실패:', m.name, text);
            }
        } catch (e) {
            failCount++;
            console.error('등록 에러:', m.name, e);
        }
    ...
    alert(`✅ 완료: ${successCount}개 등록 성공${failCount > 0 ? `, ${failCount}개 실패` : ''}`);
}

// 관리자 페이지용: 버튼 하나만 누르면 실행되는 래퍼
async function syncMaterialsFromSchedules() {
    if (!allSchedules || allSchedules.length === 0) {
        await loadSchedules();
        // 비동기라 바로 실행 안 될 수 있으니 약간 대기
        await new Promise(r => setTimeout(r, 500));
    }
    await registerUsedMaterialsToMaster();
}
